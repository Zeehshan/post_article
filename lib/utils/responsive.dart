...
class Screen {
  ...
  static double getWidth(BuildContext c) {
    double temp;
    temp = width(c);
    return temp;
  }

  static double getHeight(BuildContext c) {
    double temp;
    // if (isLandscape(c)) {
    //   temp = width(c);
    // } else {
  ...
  static double getSize(BuildContext c, double num) {
    double tempSize =
        sqrt((getHeight(c) * getHeight(c)) + (getWidth(c) * getWidth(c))) *
            (num / 100);
    return tempSize;
  }
